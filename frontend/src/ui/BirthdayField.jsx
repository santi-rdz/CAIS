...
export default function BirthdayField({
  ...
  const requiredMessage = birthdate ? 'Ingresa la fecha de nacimiento' : 'Ingresa la fecha'
  ...
  return (
    <FormRow className="w-full" htmlFor={name} label={label} required={showRequired}>
      <DatePickerComponent
        name={name}
        control={control}
        rules={{
        ...
        maxDate={maxDate}
      />
      {birthdate && edad !== null && <p className="mt-1 text-xs text-zinc-500">{edad} años</p>}
    </FormRow>
  )
}
