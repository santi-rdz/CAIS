...
export default function Input({ hasError, suffix, variant = 'fill', size = 'lg', offset, className, ...props }) {
  return (
    <div className={`relative ${className} `}>
      <input
        className={`text-4 w-full duration-100 placeholder:text-[#808080] ${hasError ? 'outline-[1.5px] outline-offset-2 outline-red-400' : ' focus-visible:outline-[1.5px] focus-visible:outline-green-900'} ${sizes[size]} ${variants[variant]} `}
        {...props}
      />
      {suffix &&
        cloneElement(suffix, {
