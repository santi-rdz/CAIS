...
import { buildAdminCreateSchema, adminEditSchema, adminSignupFormSchema } from '@schemas/users'
...
import AdminPersonalInfoForm from '@features/users/AdminPersonalInfoForm'
...
const PERSONAL_FIELDS = ['nombre', 'apellidos', 'correo', 'fecha_nacimiento', 'telefono']
...
export default function AdminForm({
  ...
  const createFormSchema = buildAdminCreateSchema(correoField)

  const stepsFields = isEdit
    ? [PERSONAL_FIELDS]
    : [PERSONAL_FIELDS, registration ? ['password', 'confirmPassword'] : ['password']]

  const defaultValues = isEdit ? parseUserDefaults(user) : registration ? { correo: email } : {}

  const resolver = isEdit
    ? zodResolver(adminEditSchema)
    : zodResolver(registration ? adminSignupFormSchema : createFormSchema)
    ...
    <FormProvider {...methods}>
      {registration ? (
        ...
        <>
          {isEdit && (
            <Modal.Heading>
              <Modal.Title>{title ?? 'Editar administrador'}</Modal.Title>
